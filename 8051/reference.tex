...
\subsubsection{Interval Timing v. Event Counting}
Interval timing works as one would expect, where the timer is started, and then
stopped either by overflow or manually. Event counting works by
pulsing the timer when an external source pulses,
effectively counting external pulses. This is set by setting `C/T` high.
\subsubsection{13-bit Timer}

\subsubsection{16-bit Timer}
In 16-bit timer mode, THx and TLx hold the high and low bytes, respectively.


\subsubsection{8-bit Auto-Reload Timer}
In auto-reload mode, TLx acts as an 8-bit timer, while THx holds a reload value.
When TLx overflows, it is reloaded with the value in THx.
...
to its own mode 3, split-timer mode is common for generating baud-rates, since
T1 can be switched on and off at arbitraty intervals.

\subsection{Interrupts}
\begin{center}
	\begin{tabular}{r|l c c}
		INTERRUPT & FLAG & bit position & Vector address\\
		\hline
		External0 & \texttt{IE0} & \texttt{TCON.1} & 0x03\\
		External1 & \texttt{IE1} & \texttt{TCON.3} & 0x13\\
		Timer1 & \texttt{TF1} & \texttt{TCON.7} & 0x1B\\
		Timer0 & \texttt{TF0} & \texttt{TCON.5} & 0x0B\\
		Transmit & \texttt{TI} & \texttt{SCON.1} & 0x23\\
		Receive & \texttt{RI} & \texttt{SCON.0} & 0x23\\
	\end{tabular}
\end{center}
\subsubsection{Interrupt and Priority Bits}
\resizebox{3.5in}{!}{
\includegraphics{img/itrploc.png}
\includegraphics{img/priority.png}
}
